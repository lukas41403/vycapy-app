...
import { Badge, Icon, IconName, PressableScale } from '@/components/ui'
...
const QUICK_ACTIONS: QuickAction[] = [
  { id: 'odpad',    icon: 'odpady',   label: 'Zber odpadu',     prompt: 'Kedy je najbližší zber odpadu? Aké typy odpadu sa zbierajú v tomto týždni?' },
  { id: 'nahlasit', icon: 'hlasenie', label: 'Nahlásiť problém', prompt: 'Chcem nahlásiť problém v obci. Kam mám ísť v aplikácii a aké údaje budem potrebovať?' },
  { id: 'hodiny',   icon: 'time',     label: 'Úradné hodiny',   prompt: 'Aké sú úradné hodiny obecného úradu? Je dnes otvorené?' },
  { id: 'ziadost',  icon: 'document', label: 'Podať žiadosť',   prompt: 'Chcem podať žiadosť na obecný úrad. Aké možnosti mám a ako postupovať?' },
  { id: 'kontakty', icon: 'kontakty', label: 'Kontakty',        prompt: 'Aké sú kontakty na obec? Telefón, email, adresa?' },
  { id: 'pobyt',    icon: 'domov',    label: 'Trvalý pobyt',    prompt: 'Ako si vybavím prihlásenie na trvalý pobyt v obci?' },
]
    ...
    <SafeAreaView style={[styles.safe, { backgroundColor: t.background }]}>
      ...
      <View style={styles.header}>
        <View style={styles.headerTop}>
          ...
          <PressableScale onPress={zacniZnova} style={styles.resetBtn} scaleTo={0.95} accessibilityLabel="Nový chat">
            <Icon name="refresh" size={14} color="#FFFFFF" /><Text style={styles.resetText}>Nový chat</Text>
          </PressableScale>
        </View>

        <View style={styles.headerRow}>
          <View style={styles.avatar}>
            <Icon name="marta" size={28} color="#FFFFFF" />
          </View>
          <View style={{ flex: 1 }}>
      ...
      >
            ...
            <View style={styles.initialQA}>
              ...
              <View style={styles.qaGrid}>
                {QUICK_ACTIONS.map(a => (
                  ...
                  >
                    <Icon name={a.icon} size={22} color={MARTA_FIALOVA} />
        ...
        {spravy.length > 1 && !thinking && (
          <View style={[styles.stickyQA, { backgroundColor: t.surface, borderTopColor: t.borderLight }]}>
            ...
            >
              {QUICK_ACTIONS.map(a => (
                <PressableScale key={a.id} style={styles.stickyChip} scaleTo={0.95} onPress={() => poslat(a.prompt)} accessibilityLabel={a.label}>
                  <Icon name={a.icon} size={13} color={MARTA_FIALOVA} />
                  <Text style={styles.stickyChipText}>{a.label}</Text>
                </PressableScale>
              ))}
            </ScrollView>
          </View>
        ...
        <View style={[styles.inputRow, { backgroundColor: t.surface, borderTopColor: t.borderLight }]}>
          ...
          >
            <Icon name="mic" size={20} color={MARTA_FIALOVA} />
          </PressableScale>
          <TextInput
          ...
          />
          <PressableScale
            style={[styles.sendBtn, (!input.trim() || thinking) && { backgroundColor: t.border }]}
            scaleTo={0.9}
    ...
    <View style={[styles.bublinaRow, jeUser && styles.bublinaRowUser]}>
      {!jeUser && (
        <View style={styles.bublinaAvatar}>
          <Icon name="marta" size={16} color={MARTA_FIALOVA} />
        </View>
      )}
      <View
        style={[
...
const styles = StyleSheet.create({
  ...
  resetText: { color: '#FFFFFF', ...typo.micro },
  ...
  stickyChipText: { ...typo.caption, fontFamily: 'Inter_700Bold', color: MARTA_FIALOVA },
